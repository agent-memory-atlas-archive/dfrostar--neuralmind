...
def generate_card(
    ...
    if share_text is None:
        share_text = f"My {repo_name} codebase scored {fmt_ratio(reduction)} fewer tokens with NeuralMind"
...
def main() -> None:
    ...
    if args.json:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "evals.public.run", "--json", "--repo", args.repo, "--seeds", str(args.seeds)],
    ...
    share_text = args.share_text or f"My {repo.get('name', 'unknown')} codebase scored {fmt_ratio(naive.get('mean_tokens', 0) / nm.get('mean_tokens', 1))} fewer tokens with NeuralMind"
    print(f"\nShare text:\n{share_text}")
    print(f"\nX/Twitter share URL:")
    print(f"https://twitter.com/intent/tweet?text={share_text.replace(' ', '%20').replace('×', 'x')}")
